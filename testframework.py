...
class TestbTCPFramework(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.server_process = Process(target=server_app.run_server, args=(winsize, timeout))
        self.client_process = Process(target=client_app.send_data, args=(self.file_to_send, winsize, timeout))
    ...
    def test_ideal_network(self):
        ...
        file_sent = open("input.file", "r")
        file_received = open("output.file", "r")
        well_received = file_sent.read() == file_received.read()
        file_sent.close()
        file_received.close()

        self.assertTrue(well_received)
